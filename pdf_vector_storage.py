        ...
        if new_or_changed_files:
            ...
            for pdf_file in new_or_changed_files:
                ...
                try:
                    ...
                    # Create settings that avoid costly enrichment — just do text embedding
                    add_settings = Settings()
                    add_settings.parsing.multimodal = MultimodalOptions.OFF  # Disable image/table enrichment
                    add_settings.embedding = self.embedding_model  # Use the configured embedding model
